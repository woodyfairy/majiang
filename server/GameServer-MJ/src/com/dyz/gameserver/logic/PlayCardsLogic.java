...
public class PlayCardsLogic {
     ...
     * 有人听牌的数组
     */
    private List<Avatar> tingAvatar = new ArrayList<>(); 
    /**
     * 存抓的码
     */
        ...
        if(avatar.getRoomVO().getZiMo() == 0 && !avatar.getRoomVO().getHong()){
        	...
        	for(int i=0;i<playerList.size();i++){
        		...
        		if(ava.getUuId() != avatar.getUuId()) {
        			...
        			if ((!ava.avatarVO.isTing()) && ava.checkGang(putOffCardPoint)) {
        				gangAvatar.add(ava);
        				//同时传会杠的牌的点数
        				sb.append("gang:"+putOffCardPoint+",");
        			}
        			if ((!ava.avatarVO.isTing()) && ava.checkPeng(putOffCardPoint)) {
        				penAvatar.add(ava);
...
//    							 lastAvtar = playerList.get(i);
    ...
    private boolean checkTing(Avatar avatar, boolean outCard, List<Integer> tingList)
    {
    	...
    	{
    		if(rawPai[1][i] == 2) continue;
    		if(rawPai[1][i] == 1 && rawPai[0][i] == 3) continue;
    	...
    	{
    		System.out.println(" e=" + tingList.get(i).toString());
    	}
    	...
    	for (Avatar ava : playerList) {
			if(ava.getUuId() != avatar.getUuId() && ava.qiangHu){
				...
				if(roomVO.getRoomType() == 2){//划水麻将
					...
					//在后面的胡牌信息里面修改的分数
					if(checkHuZZhuan(ava, cardPoint)){
						huAvatar.add(ava);
						//向玩家发送消息
						ava.getSession().sendMsg(new ReturnInfoResponse(1, "qianghu:"+cardPoint));
						flag = true;
    ...
    public boolean validateStatus(){
    	if(tingAvatar.size() > 0 || huAvatar.size() > 0 || penAvatar.size()>0 || gangAvatar.size()>0 || chiAvatar.size()>0 ||qishouHuAvatar.size()>0){
    		return true;
    	}
    	else{
    		return false;
